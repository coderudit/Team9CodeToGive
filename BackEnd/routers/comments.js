...
const { updateTableRow, userIsModerator } = require("../database/utils");
...
router.post("/", auth, async (req, res) => {
  try {
    ...
    const {
      rows: [{ id }],
    } = await query(insertCommentStatement, [
      body,
      req.user.id,
      post_id,
      parent_comment_id,
    ...
    const {
      rows: [comment],
    } = await query(selectInsertedCommentStatement, [req.user.id, id]);
